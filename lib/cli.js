...
import { clamp, color, errorsContext, getRuntime, isPrivateIPv4 } from './utils.js';
...
export async function run() {
	...
	const argOptions = parseArgs(args, context);
	const options = serverOptions(argOptions, context);

	if (context.errors.length) {
		logger.writeErrors(context.errors);
		logger.write('error', `Try 'servitsy --help' for more information.`);
		if (context.errors.some((err) => err.error)) {
			process.exitCode = 1;
			return;
		}
	}
...
export function helpPage() {
	...
	const optionCols = (options, { gap, firstWidth }) =>
		options.flatMap(({ help, names, default: argDefault = '' }) => {
			...
			const second = style(secondRaw, 'gray');
