...
@typedef {import('../types.js').FSEntryBase & { isParent?: boolean }} DirListEntry
**/
...
/**
 * @param {{ dirPath: string; urlPath: string, items: DirListEntry[] }} data
...
export function dirListPage({ dirPath, urlPath, items }, options) {
	...
	const sorted = [
		...items.filter((x) => x.kind === 'dir'),
		...items.filter((x) => x.kind === 'link'),
		...items.filter((x) => x.kind === 'file' || x.kind == null),
	];
	if (trimmedPath !== '') {
		sorted.unshift({
...
/**
 * @param {DirListEntry} item
 * @param {Pick<ServerOptions, 'ext'>} options
 * @returns {string}
 */
function dirListItem({ filePath, kind, isParent = false }, { ext }) {
