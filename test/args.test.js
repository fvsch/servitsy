...
import { errorsContext, intRange } from '../lib/utils.js';
...
suite('parseArgs', () => {
	...
	test('validates --port syntax', () => {
		...
		deepStrictEqual(context.errors, [
			{ warn: `invalid --port value: '+1000'` },
			{ warn: `invalid --port value: 'whatever'` },
			{ warn: `invalid --port value: '{"some":"json"}'` },
		]);
	});
	...
	test('rejects invalid --header syntax', () => {
		...
		deepStrictEqual(context.errors, [
			{ warn: `invalid --header value: 'basic string'` },
			{ warn: `invalid --header value: '*.md {"bad": [json]}'` },
		]);
	});
